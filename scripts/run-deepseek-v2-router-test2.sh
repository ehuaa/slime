#!/bin/bash
# ROUTER-POLICY A/B TEST (round 2): identical to run-deepseek-v2-multinode.sh EXCEPT
#   - --sglang-router-policy taken from $1 (round_robin | power_of_two | ...)
#   - eval skipped (EVAL_ARGS empty), num-rollout 1, wandb off, save -> scratch dir
# Baseline to beat (random policy, 2026-07-07 step-0): perf/rollout_time 4059.97s.
# rollout-shuffle seed is fixed -> same 128-prompt first wave as the baseline run.
...
set -ex
POLICY=${1:?usage: run-deepseek-v2-router-test2.sh <router-policy>}
...
CKPT_ARGS=(
   ...
   --ref-load "${HF_CKPT}"
   --load "${HF_CKPT}"
   --save /mnt/data/data/home/czh/RL/DeepSeek-V2-021A_router_test/
