#!/bin/bash
# SMOKE TEST variant of run-deepseek-v2-opd.sh: 4 nodes / 32 GPUs (first exercise of
# the dp2/edp2 colocate layout on this cluster), 2 rollouts, small batch (rbs 8 x n 4
# = GBS 32), 16384 response cap, shrunken eval (aime x1 sample, deepscaler-scored),
# no wandb / no checkpoint saves. Validates: teacher HF-bridge load via
# --opd-teacher-load, teacher/ref host-backup swap forwards, rm_type zero + eval
# rm override, R3, and the rollout -> train -> rollout -> train cycle.
...
CKPT_ARGS=(
   ...
   --ref-load "${HF_CKPT}"
   --load "${HF_CKPT}"
   --save /mnt/data/data/home/czh/RL/DeepSeek-V2-021A_opd_smoke/
...
EVAL_ARGS=(
   ...
   --eval-config "${SLIME_DIR}/scripts/eval-config-deepseek-v2-smoke.yaml"
