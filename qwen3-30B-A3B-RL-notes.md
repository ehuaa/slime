...
### 结果(训练有效)
- **eval/aime**:`0.8125(s0) → 0.831(s20) → 0.835(s40) → 0.842(s60) → 0.831(s80)` —— 整体爬升到 ~0.83-0.84(小样本 30 题有 ±0.01~0.02 噪声)。
- **raw_reward**:`0.76 → ~0.82`,缓慢上行。
- **train_rollout_logprob_abs_diff**:全程稳定 ~0.017(训推高度对齐)。
- **grad_norm ~0.04、entropy ~0.24** 平稳,无坍缩;**ppo_kl=0**(on-policy 预期)。
